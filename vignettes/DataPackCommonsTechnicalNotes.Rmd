...
binded_model <- rbindlist(your_model)
...
Once we know all indicator are present we can move on to checking the age/sex/kp disaggs. There is a handy function in the package `checkModelDisagg` to help us isolate problem areas. Below is an example analysis that looks at age disaggs to identify a missing disagg value present in the schema but missing in the model:
...
# run disagg function
res <- checkModelDisagg(binded_model, "age")

# lets assume the report res shows HTS_TST.Pos.Total_With_HEI.T_1 is showing a missing
# id in age that is present in schema but not in model

# check missing model data reported
res %>% filter(indicator_code == "HTS_TST.Pos.Total_With_HEI.T_1")

#                   indicator_code                        msg
#1: HTS_TST.Pos.Total_With_HEI.T_1 model missing: sMBMO5xAq5T

# check schema to make sure
filter(datapackr::cop24_data_pack_schema,
       (dataset == "mer" & col_type == "past") |
         (dataset == "datapack" & col_type == "calculation")) %>%
  filter(indicator_code == "HTS_TST.Pos.Total_With_HEI.T_1") %>%
  pull(valid_ages)
...
# check in model
binded_model %>% filter(indicator_code == "HTS_TST.Pos.Total_With_HEI.T_1") %>%
  select(indicator_code, age_option_uid, value) %>%
  distinct(age_option_uid) %>%
  pull(age_option_uid)

#[1] "A9ddhoPWEUn" "p3kqRd8LE4a" "I0g0vpEQ3UB" "Sga7ddy3GYG" "TpXlQcoXGZF" "jcGQdcpPSJP"
```

So we can see that in this case `sMBMO5xAq5T` is indeed missing from the model and this must be investigated.

In addition to searching for the missing age_option_id in the case above, wee can group model output data and validate the data against the pivot tables in the DATIM. Below we summarize the age_option_uid data for matching against DATIM; note `<1` is missing which is the category option id we flagged in the previous code chunk:
...
age_names <- age_names[[1]][[1]] %>%
  rename(age_option_uid = id)

binded_model %>% filter(indicator_code == "HTS_TST.Pos.Total_With_HEI.T_1") %>%
  select(indicator_code, period, age_option_uid, value) %>%
  group_by(age_option_uid, period) %>%
  summarise(total = sum(value)) %>%
  inner_join(
    age_names
  ) %>%
  select(name, age_option_uid, period, total) %>%
  arrange(name)

# A tibble: 6 × 4
# Groups:   age_option_uid [6]
#   name  age_option_uid period   total
#   <chr> <chr>          <chr>    <dbl>
# 1 01-09 A9ddhoPWEUn    2023Oct  83086
# 2 10-14 jcGQdcpPSJP    2023Oct  42269
# 3 15-24 p3kqRd8LE4a    2023Oct 216867
# 4 25-34 I0g0vpEQ3UB    2023Oct 391622
# 5 35-49 Sga7ddy3GYG    2023Oct 680880
# 6 50+   TpXlQcoXGZF    2023Oct 408372
```

Another use case for this function is searching for data in the model that is not present in the schema; datapack model generation does not care if there is extra data in the model as it only keeps what's needed from the schema, but below we pull flagged records and pivot the table to get an easily sortable result:

```{r, eval=F}

# analysis of extra model age disaggs not in schema ----
extra_mod_disaggs <- res %>% 
  filter(grepl("BUT", msg)) %>% 
  separate(msg,into=c("msg1", "ids"), 
           sep="\\:", convert = TRUE, extra = "merge")

nmax <- max(stringr::str_count(extra_mod_disaggs$ids, "\\,")) + 1

extra_mod_disaggs_p <- 
  extra_mod_disaggs %>% 
  separate(ids, paste0("col", seq_len(nmax)), sep = "\\,", fill = "right") %>%
  select(-msg1) %>%
  pivot_longer(-indicator_code) %>%
  na.omit() %>%
...
#  A tibble: 23 × 3
#    indicator_code            name  value      
#    <chr>                     <chr> <chr>      
#  1 CXCA_SCRN.R               col1  p3kqRd8LE4a
#  2 CXCA_SCRN.R               col2  TpXlQcoXGZF
#  3 CXCA_SCRN.R               col3  A9ddhoPWEUn
#  4 CXCA_SCRN.R               col4  jcGQdcpPSJP
#  5 CXCA_SCRN.R               col5  sMBMO5xAq5T
#  6 CXCA_SCRN.Rt.R            col1  p3kqRd8LE4a
#  7 CXCA_SCRN.Rt.R            col2  TpXlQcoXGZF
#  8 HTS.Index.Pos.Yield       col1  sMBMO5xAq5T
#  9 HTS_TST.ActiveOther.Yield col1  sMBMO5xAq5T
# 10 HTS_TST.Other.Yield       col1  sMBMO5xAq5T
# … with 13 more rows
# ℹ Use `print(n = ...)` to see more rows

# quick sanity check for an example like CXCA_SCRN.R 
filter(datapackr::cop24_data_pack_schema,
       (dataset == "mer" & col_type == "past") |
         (dataset == "datapack" & col_type == "calculation")) %>%
  filter(indicator_code == "CXCA_SCRN.R") %>%
  pull(valid_ages)
...
So we can see in this case the schema calls for two disaggregations for `CXCA_SCRN.R` but the model is showing more disaggregations.
