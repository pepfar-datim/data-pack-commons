...
For example below we can use the following function `checkMissingIndicators` to isolate indicator_codes present in the schema but missing a datpapack model:

```{r, eval=FALSE}

you_model <- readRDS("your model ...")

# which indicators don't show up in the model?
missing_model_data <- checkMissingIndicators(model = your_model) %>% filter(row_count < 1)
unique(missing_model_data$indicator_code)

#[1] "HTS_TST.KP.Pos.Yield.T"

# pull schema data for missing indicator to understand more
missing_data <- datapackr::cop24_data_pack_schema %>%
  filter(indicator_code %in% unique(missing_model_data$indicator_code))
```

Note that while the code above may show indicators which are missing, this does not mean they need to be added, check previous years to make sure there is already a reason for a missing indicator, in this case `HTS_TST.KP.Pos.Yield.T` is noted as an indicator_code we did not currently need to add. 
...
Another use case for this function is searching for data in the model that is not present in the schema. Below we pull flagged records and pivot the table to get an easily sortable result:
