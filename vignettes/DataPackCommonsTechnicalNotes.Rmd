...
Once we know all indicator are present we can move on to checking the age/sex/kp disaggs. There is a handy function in the package `createDisaggReport` to help us isolate problem areas. Below is an example analysis that looks at age disaggs to identify a missing disagg value present in the schema but missing in the model (results are included hashed as model and schema may change):
...
# run disagg function
res <- createDissagReport(binded_model, cop_year = 2024)

# look for any missing values in disaggs
res$mismatched_report_uniques %>% 
  select(indicator_code, sex, age, kp) %>%
  filter(grepl("MISSING", age) | grepl("MISSING", sex) | grepl("MISSING", kp))

#                   indicator_code sex                        age kp
#1: HTS_TST.Pos.Total_With_HEI.T_1     MODEL MISSING: sMBMO5xAq5T 

# we can double check visually the indicators
res$mismatched_report_uniques %>%
  filter(indicator_code == "HTS_TST.Pos.Total_With_HEI.T_1") %>%
  select(indicator_code, valid_a, model_a)

#                   indicator_code                                                                                   valid_a
# 1: HTS_TST.Pos.Total_With_HEI.T_1 A9ddhoPWEUn, I0g0vpEQ3UB, jcGQdcpPSJP, p3kqRd8LE4a, Sga7ddy3GYG, sMBMO5xAq5T, TpXlQcoXGZF
#                                                                         model_a
# 1: A9ddhoPWEUn, I0g0vpEQ3UB, jcGQdcpPSJP, p3kqRd8LE4a, Sga7ddy3GYG, TpXlQcoXGZF
```

So we can see that in this case `sMBMO5xAq5T` is indeed missing from the model.
